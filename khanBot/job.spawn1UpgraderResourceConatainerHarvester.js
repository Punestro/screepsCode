var jobSpawn1UpgraderResourceConatainerHarvester = {
    ...
    if(creep.transfer(turret, RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
      ...
      return 'Turret attack!';}}
	    ...
	    if(creep.store.getFreeCapacity() > 0 ) {
            var sources = Game.getObjectById('Memory.spawn1UpgraderResource');
            if(creep.harvest(sources) == ERR_NOT_IN_RANGE) {
                creep.moveTo(sources), {visualizePathStyle: {stroke: '#f8ffab'}};
        }
        }
        else {
            var targets = Game.getObjectById('Memory.containerSpawn1Upgrader')            
            if(targets) {
...
module.exports = jobSpawn1UpgraderResourceConatainerHarvester;
